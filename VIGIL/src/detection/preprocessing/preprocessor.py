# from  preprocessing.detection.retinaface.retinaface import  RetinaFace
import datetime
import logging
import os
...
from preprocessing.quality.models.msba import MSBA
...
class AlignFacePreprocess(object):
    def __init__(self, config: Union[str, dict]):
        ...
        # self.face_evalues = MSBA() # GPU
        predictor_path = osp.join(
    ...
    def __call__(self, image: np.ndarray, infer=None):
        cropped_face, landmarks, _ = self.extract_aligned_face_dlib(image, infer=infer)
        return cropped_face

    def extract_aligned_face_dlib(
        self, image: np.ndarray, res: int = 224, mask: np.ndarray = None, infer=None
    ):
        def img_align_crop(img, landmark=None, outsize=None, scale=1.3, mask=None):
            ...
            if outsize is not None:
                img = cv2.resize(
                    img, (outsize[1], outsize[0]), interpolation=cv2.INTER_CUBIC
                )
        ...
        if len(faces):
            ...
            # cropped_face = self.face_evalues(cropped_face, infer)
            # landmark = face_utils.shape_to_np(landmark)

            cropped_face = cv2.cvtColor(cropped_face, cv2.COLOR_RGB2BGR)
            return cropped_face, None, mask_face
