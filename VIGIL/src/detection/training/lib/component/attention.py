...
class DualCrossModalAttention(nn.Module):
    ...
    def forward(self, x, y):
        ...
        def _get_att(a, b):
            proj_key1 = (
                self.key_conv_share(self.key_conv1(a))
                .view(B, -1, H * W)
                .permute(0, 2, 1)
            )  # B , HW, C
            ...
            attention2 = self.softmax(
                self.linear2(energy.permute(0, 2, 1))
            )  # BX (N) X (N)
        ...
        proj_value_y_on_x = self.value_conv2(y).view(B, -1, H * W)  # B , C , HW
        ...
        proj_value_x_on_y = self.value_conv1(x).view(B, -1, H * W)  # B , C , HW
...
class DualCrossModalAttention_old(nn.Module):
    ...
    def forward(self, x, y):
        ...
        def _get_att(q, k):
            proj_query = (
                self.query_conv(q).view(B, -1, H * W).permute(0, 2, 1)
            )  # B , HW, C
            proj_key = self.key_conv(k).view(B, -1, H * W)  # B X C x (*W*H)
