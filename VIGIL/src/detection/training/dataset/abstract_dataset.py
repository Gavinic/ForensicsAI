...
class DeepfakeAbstractBaseDataset(data.Dataset):
    ...
    def __init__(self, config=None, mode="train"):
        ...
        ## Check once whether the paths exist
        for file_path, label in zip(image_list, label_list):
            if os.path.exists(os.path.join(self.config["rgb_dir"], file_path)):
        ...
        self.lenght = len(self.image_list)

        self.transform = self.init_data_aug_method()

    def mixup_partial(self, img1, lb1, alpha=0.4):
        ## If forged, sample randomly; if real, only blend with real
        if lb1 == 1:
            indix2 = random.randint(0, self.lenght - 1)
            image_paths2 = self.data_dict["image"][indix2]
            label2 = self.data_dict["label"][indix2]
        else:
            indix2 = random.randint(0, len(self.real_list) - 1)
            image_paths2 = self.real_list[indix2]
            label2 = 0

        image2 = np.array(self.load_rgb(image_paths2))
        l_eye = np.array([115, 131, 80, 110])
        r_eye = np.array([115, 131, 145, 168])
        mouth = np.array([177, 205, 105, 162])
        lam = np.random.beta(alpha, alpha)
        # jitter
        l_eye += np.random.randint(-5, 5, 4)
        r_eye += np.random.randint(-5, 5, 4)
        mouth += np.random.randint(-5, 5, 4)
        if random.random() < 0.5:
            img1[l_eye[0] : l_eye[1], l_eye[2] : l_eye[3]] = (
                lam * img1[l_eye[0] : l_eye[1], l_eye[2] : l_eye[3]]
                + (1 - lam) * image2[l_eye[0] : l_eye[1], l_eye[2] : l_eye[3]]
            )
        img1[r_eye[0] : r_eye[1], r_eye[2] : r_eye[3]] = (
            lam * img1[r_eye[0] : r_eye[1], r_eye[2] : r_eye[3]]
            + (1 - lam) * image2[r_eye[0] : r_eye[1], r_eye[2] : r_eye[3]]
        )
        if random.random() < 0.35:
            img1[mouth[0] : mouth[1], mouth[2] : mouth[3]] = (
                lam * img1[mouth[0] : mouth[1], mouth[2] : mouth[3]]
                + (1 - lam) * image2[mouth[0] : mouth[1], mouth[2] : mouth[3]]
            )
        # lb = lb1*lam + lb2*(1-lam)
        lb = lb1 | label2
        return img1, lb
        ...
        trans = A.Compose(
            [
                ...
                A.OneOf(
                    ...
                    p=0,
                ),  # 0 if self.config['with_landmark'] else  1
                A.RandomScale(
                    scale_limit=(-0.5, 0), p=0.3
                ),  # shrink the image to 50%-100% of its original size
                A.PadIfNeeded(
                ...
                A.OneOf(
                    [A.CoarseDropout(max_width=16, max_height=16, p=1)],
                    p=self.config["data_aug"]["GridDropout"],
                ),
                A.ImageCompression(
    ...
    def collect_img_and_label_for_one_dataset(self, dataset_name: str):
        ...
        if not os.path.exists(self.config["dataset_json_folder"]):
            self.config["dataset_json_folder"] = self.config[
                ...
                "<BASE_PATH>/Youtu_Pangu_Security_Public",
                "<BASE_PATH>/Youtu_Pangu_Security/public",
            )
        try:
            with open(
                os.path.join(
        ...
        if isinstance(dataset_info, dict):
            ...
            video_name_list.extend(real_methods)
            video_name_list.extend(fake_methods)

            real_labels = [0] * len(real_frames)
            fake_labels = [1] * len(fake_frames)

            # self.real_list.extend(real_frames)
            # self.fake_list.extend(fake_frames)
        ...
        if not self.lmdb:
            ...
            with self.env.begin(write=False) as txn:
                ...
                if image_bin is None:
                    ...
                    image_buf = np.frombuffer(image_bin, dtype=np.uint8)
                    # cv2.IMREAD_GRAYSCALE for grayscale, cv2.IMREAD_COLOR for color
    ...
    def __getitem__(self, index, no_norm=False):
        ...
        for image_path in image_paths:
            ...
            if self.mode == "train" and self.config["use_data_augmentation"]:
                ## Add a mixup
                # if random.random() < self.config['mixup']:
                #     image,label = self.mixup_partial(image,label)
        ...
        return image_tensors, label, landmark_tensors, mask_tensors, image_path
...
if __name__ == "__main__":
    with open(
        "<BASE_PATH>/DeepfakeBench/training/config/detector/video_baseline.yaml", "r"
    ) as f:
