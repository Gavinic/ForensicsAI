...
from utils.cutmix import cutmix, cutmix_for_sam
        ...
        for epoch in range(self.num_epochs):
            ...
            for i, data in enumerate(data_loaders):
                ...
                if self.sam:
                    ...
                    inputs, lam, target_a, target_b = cutmix_for_sam(
                        inputs, labels, cutmix_prob=0.8, beta=0.5
                    )
                    ...
                    (
                        criterion(model(inputs), target_a) * lam
                        + criterion(model(inputs), target_b) * (1.0 - lam)
                    ).mean().backward()
                    optimizer.second_step(zero_grad=True)
                    _, preds = torch.max(outputs.data, 1)
                else:
                    outputs, loss = cutmix(
                        inputs, labels, model, criterion, cutmix_prob=0.8, beta=0.5
                    )
                    _, preds = torch.max(outputs.data, 1)
                    optimizer.zero_grad()
                    loss.backward()
