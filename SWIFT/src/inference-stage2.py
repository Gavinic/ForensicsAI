...
DEFAULT_USER_QUERY = (
    "Perform forgery detection on this image and output a forensic report. "
    "You must output only one JSON object; do not output extra text, Markdown, or code blocks. "
    "The JSON must contain the three fields label, bbox, and explanation. "
    "label: 0 means real, 1 means forged. "
    "bbox: a list of normalized coordinates for all tampered regions; coordinates must be in the range [0, 1000], formatted as [[x1, y1, x2, y2], ...]; output [] if there are no tampered regions. "
    "explanation: explain your forensic reasoning in a single natural-language paragraph; do not use lists. "
    "If the main subject is a receipt or bill, focus on checking numbers, decimal places, tax-amount calculations, font consistency, and time/date logic. "
    "If the main subject is a person, object, or scene, focus on checking fingers, faces, text, reflections, edges, textures, and lighting/shadows for generative-forgery or tampering traces. "
    "If the image has overlaid guiding Chinese subtitles unrelated to the original content, also judge it as forged and provide the subtitle region in bbox."
)
